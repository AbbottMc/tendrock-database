import { databaseManager, InstanceData } from "../database";
import { world } from "@minecraft/server";
import { DataTest } from "./DataTest";
DataTest.init();
databaseManager.setFlushInterval(6 * 20, false);
// 继承自 `InstanceData` 抽象类
export class CustomConstructor extends InstanceData {
    // *[必要] 实现抽象方法，加载实例时调用
    ...
    onDeserialize(dataJson, instanceDataOptions, options) {
    ...
    // @param options - 开发者自定义的选项参数
    // @param instanceDataOptions - 实例数据初始化选项参数，提供了初始化对象时的一些运行时参数
    onConstruct(options, instanceDataOptions) {
        var _a, _b, _c;
        // 如果需要设定默认初始值，请在这里初始化数据
        this._customData = (_a = options.initialCustomData) !== null && _a !== void 0 ? _a : 'Initial Data';
        this._extraParam = options.extraParam;
        this._extraParamLength = (_c = (_b = this._extraParam) === null || _b === void 0 ? void 0 : _b.length) !== null && _c !== void 0 ? _c : 0;
    }
    // *[必要] 实现抽象方法，新建实例且 options 和 dataJson 都为 undefined 时调用
    // @param instanceDataOptions - 实例数据初始化选项参数，提供了初始化对象时的一些运行时参数
    onInitWithNoData(instanceDataOptions) {
...
world.afterEvents.playerPlaceBlock.subscribe(({ block }) => {
    ...
    databaseManager.createDataInstanceIfAbsent(block, 'test:test_id', CustomConstructor, {
...
world.afterEvents.playerBreakBlock.subscribe(({ block }) => {
    ...
    const customDataInstance = databaseManager.getBuiltDataInstance(block, 'test:test_id');
